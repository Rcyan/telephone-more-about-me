...
(function($, root) {
	...
	$.extend(CAP.ui.BasicDiaglog.prototype, {
		...
		init: function(options) {
			var self = this,
			tpl = '<h3 class="yzm-tit" style="line-height:30px;padding-left:10px;font-size:12px;font-weight:bold;background-color:#f2f2f2;color:#333;">璇疯緭鍏ラ獙璇佺爜</h3><a href="javascript:void(0);" class="close-a">鍏抽棴</a>',
			$window = $(window);
	...
	CAP.ui.normal_captcha = function( options ) {
		var obj = {},
			...
			_dialog = null,
			$btn = null;
		tpl += '<p><input type="text" class="yzm-text" value="" placeholder="鎸変笅鍥捐緭鍏ラ獙璇佺爜锛屼笉鍖哄垎澶у皬鍐�" title="鎸変笅鍥捐緭鍏ラ獙璇佺爜锛屼笉鍖哄垎澶у皬鍐�"/></p>';
		tpl += '<p><img src="" alt="" />鐪嬩笉娓咃紵<a href="javascript:void(0);" class="chose-btn-vote">鎹竴寮�</a></p><p class="tc">';
		tpl += '<input type="button" class="sub-btn" value="鎻愪氦" id="'+btn_id+'" /></p></div>';
		obj.content = tpl;
	...
	CAP.ui.ads_captcha = function( options ){
		...
		options || {});
		tpl += '<p><a href="javascript:void(0);" class="chose-btn chose-btn-adcaptcha">鎹竴寮�</a><i class="icon-q captcha-q">&nbsp;</i><span>璇疯緭鍏ヤ笅鍥句腑鏈湡璁胯皥鍢夊鐨勫鍚嶆槸浠€涔堬紵</span></p>';
		tpl += '<div class="yzm-img" style="text-align:center;"><img src="'+loading+'" alt="" /></div><p><i class="icon-a">&nbsp;</i><input type="text" class="yzm-text" value="" placeholder="璇峰湪姝よ緭鍏ユ纭瓟妗�" title="璇峰湪姝よ緭鍏ユ纭瓟妗�"/></p><p class="tc">';
		tpl += '<span class="captcha-error" style="color:red;"></span><input type="button" class="sub-btn" value="鎻愪氦" id="'+btn_id+'" /></p></div>';
		obj.content = tpl;
		_dialog = new CAP.ui.BasicDiaglog(obj);
	...
	CAP.ui.captcha = function(options){
		...
		$.get("http://vote.act.city.sina.com.cn/interface/activity/json_get_act_info.php?act_id="+options.act_id, {}, function(json){
			if(json.error==1){
				...
				act_info = json.data;
				if(!act_info){
					err('<span style="color:red;">娲诲姩涓嶅瓨鍦�</span>');
					return false;
				};
				//澧炲姞楠岃瘉鐮佺被鍨嬪弬鏁�;榛樿涓烘姇绁ㄧ被鍨�;
